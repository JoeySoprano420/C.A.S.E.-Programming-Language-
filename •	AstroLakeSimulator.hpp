...
// Rename to avoid conflicts with transpiler's Node
namespace AstroLake {
    ...
    struct ThreadDescriptor {
        ...
        double perfPerWatt;
        int socketID;
        int x; // Added for grid positioning
    };
    ...
    struct AIBalancer {
        ...
        double predict(double temp, double perf) {
            double x = wPerf * perf - wTemp * (temp - 30) + bias + memory * 0.1;
            memory = std::tanh(x);
            return std::clamp(1.0 + memory, 0.3, 1.8);
        }
