...
// Consolidated includes at the top
#include <cctype>
#include <fstream>
#include <iostream>
#include <sstream>
...
void MacroRegistry::persist() {
    // For C++14 compatibility, replace std::filesystem with manual directory creation if needed
    // Assuming boost::filesystem or manual mkdir; for simplicity, assume directory exists or use system("mkdir build");
...
static void emitNode(Node* n, std::ostringstream& out) {
    if (n->type == "Program") {
        ...
        std::string keyTy = n->children[0]->value;
        std::string valTy = n->children[1]->value;
        ...
        out << "{ auto __f = std::async(std::launch::async, [=](){\n";
        if (!n->children.empty()) emitChildren(n->children[0]->children, out);
        out << "}); __f.get(); }\n";
    }
    else if (n->type == "Batch") {
        out << "#if defined(_OPENMP)\n#pragma omp parallel for\n#endif\n";
        out << "for(size_t __i=0; __i<" << n->value << "; ++__i){\n";
        if (!n->children.empty()) emitChildren(n->children[0]->children, out);
        out << "}\n";
    }
