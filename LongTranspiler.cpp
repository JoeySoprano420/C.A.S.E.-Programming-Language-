...
int main(int argc, char** argv) {
    ...
    if (argc > 1 && std::string(argv[1]) == "--case") {
        if (argc < 3) {
            std::cerr << "Usage: Transpiler.exe --case <source.case> [output]\n";
            return 1;
        }
        
        CASE::CASECompiler::CompilerOptions; options;
        options.inputFile = argv[2];
        options.outputFile = (argc > 3) ? argv[3] : "program";
        options.verbose = true;
        options.enableCIAM = true;
        options.emitLLVM = true;
        
        bool success = CASE::CASECompiler::compile(options);
        return success ? 0 : 1;
    }
